<template>
    <div class="header">
        ...
        <a-menu v-model="current" mode="horizontal" theme="light">
            ...
            <a-menu-item key="2" @click="jumpToUserInfo" v-if="this.userInfo.userType==='Client'">
                <a-icon type="user" />个人中心
            </a-menu-item>
            <a-menu-item key="3" @click="selectMenu" v-if="this.userInfo.userType!=='Client'">
                <router-link :to="{ name: 'manageHotel'}">
                     <a-icon type="switcher" />酒店管理
                </router-link>
            </a-menu-item>
            <a-menu-item key="4" @click="selectMenu" v-if="this.userInfo.userType==='Admin'">
                <router-link :to="{ name: 'manageUser'}">
                     <a-icon type="user" />账户管理
                </router-link>
            </a-menu-item>
            <a-menu-item key="5" @click="selectMenu" v-if="this.userInfo.userType==='Marketer'">
                <router-link :to="{ name: 'manageClient'}">
                    <a-icon type="team" />客户管理
                </router-link>
            </a-menu-item>
        </a-menu>
